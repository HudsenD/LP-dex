...
contract Dex is ReentrancyGuard {
    ...
    // poolName -> token -> amount?? fuuuuuuuuuuuuck my brain dude. I gotta draw this out or somehting
    ...
    //what if i concat strings of address+poolname?? ok i think this might be big brain.
    ...
    function addLiquidity(
        ...
        // shit how do we know which token to add. we can force them to add both. or we can check which one it is. we could also do two functions. one for a one for b
        // gonna just check which one. how tf do we do that??
    ...
    // X * Y = K
    function getPrice(string calldata poolName, ERC20 baseToken) external view returns (uint256) {
        Pool memory pool = poolInfo[poolName];
