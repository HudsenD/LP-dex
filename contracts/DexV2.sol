...
contract DexV2 is ReentrancyGuard {
    ...
    function addLiquidity(string calldata poolName, ERC20 token, uint256 amount) external isOpen(poolName) {
    ...
    function removeLiquidity(string calldata poolName, ERC20 token, uint256 amount) external isOpen(poolName) {
        bytes32 userKey = generateUserKey(poolName, msg.sender);
